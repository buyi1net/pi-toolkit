// 会话短码派生（规格「会话发现与通讯」·发现·会话短码派生）：会话 id 的确定性 6 位
// Crockford base32 派生，唯一实现归 shared/——tui 短码段、peers 列表与寻址解析共用，
// 禁止在别处重复派生逻辑。
//
// 返回值不带 `#` 前缀（呈现侧自行加，规格：呈现为 `#` 加 6 位共 7 列）；
// 规范化后不是 32 位十六进制（缺失、非法字符、长度不符）返回 null，不抛错、不猜值。
